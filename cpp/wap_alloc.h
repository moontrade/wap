...
struct ParseError {
  ...
  const Code reason;
  ParseError(const Code r) : reason(r) {}
};
} // namespace wap
...
public:
  ...
  constexpr SizeClass(uint16_t idx, uint16_t s, uint16_t sz)
      : index(idx), shift(s), size(sz) {}

  inline uint32_t set(BitmapT bitmap) const noexcept {
    return bitmap | (ONE << shift);
  }

  inline uint32_t unset(BitmapT bitmap) const noexcept {
    return bitmap & ~(ONE << shift);
  }
...
template <typename SizeT> struct SizeClassArray {
  using SizeClassT = SizeClass<SizeT>;
...
public:
  ...
  constexpr SizeClassArray(const SizeT d, const SizeT mn,
                           const SizeClassT (&l)[N])
      : begin(l), size(N), min(mn), max(N > 0 ? l[N - 1].size : 0),
        boundary(max == 0 ? 0 : max + 1), div(d) {}

  constexpr SizeClassArray(const SizeClassArray<SizeT> &l) = default;
  ...
  constexpr SizeClassT operator[](int index) const noexcept {
    return begin[index];
  }
};

template <typename SizeT, SizeClassArray<SizeT> Classes,
          SizeClassArray<SizeT> Small, SizeClassArray<SizeT> Medium,
          SizeClassArray<SizeT> Large>
struct SizeClasses {
  using SizeClassT = SizeClass<SizeT>;
...
template <typename SizeT, typename Classes> class FreeList {
public:
  using BitmapT = uint32_t;
  using SizeClassT = SizeClass<SizeT>;
  using ListSizeT = SizeT;
  constexpr static const SizeT CLASS_COUNT = Classes::class_count();
  constexpr static SizeClassT get_class(int index) {
    return Classes::get_class(index);
  }
...
public:
  constexpr static inline SizeClassT alloc_size(SizeT size) {
    return Classes::alloc_size(size);
  }
  constexpr static inline SizeClassT pool_size(SizeT size) {
    return Classes::pool_size(size);
  }
  constexpr static inline SizeT get_min_size() {
    return Classes::get_min_size();
  }
  constexpr static inline SizeT get_max_size() {
    return Classes::get_max_size();
  }
  constexpr static inline SizeT get_min_split() {
    return Classes::get_min_split();
  }
  constexpr static inline bool can_split(SizeT current, SizeT size) {
  ...
  inline SizeT search(SizeT size) {
    SizeClass cls = alloc_size(size);
    if (WAP_UNLIKELY(cls.size == 0))
      return 0;
    const int index = cls.search(fl_);
    if (WAP_LIKELY(index < CLASS_COUNT))
...
private:
  class Data {
  private:
    constexpr static SizeClass<uint16_t> sizes[] = {
        {0, 31, 12},    {1, 30, 16},    {2, 29, 20},   {3, 28, 24},
        {4, 27, 28},    {5, 26, 36},    {6, 25, 44},   {7, 24, 60},
        {8, 23, 76},    {9, 22, 92},    {10, 21, 124}, {11, 20, 156},
        {12, 19, 188},  {13, 18, 220},  {14, 17, 252}, {15, 16, 300},
        {16, 15, 364},  {17, 14, 428},  {18, 13, 508}, {19, 12, 764},
        {20, 11, 1020}, {21, 10, 1280}, {22, 9, 1532}, {23, 8, 2044},
        {24, 7, 4092},  {25, 6, 8188},  {26, 5, 16380}};
    constexpr static SizeClass<uint16_t> small[]{
        {0, 31, 12},   {0, 31, 12},   {0, 31, 12},   {0, 31, 12},
        {1, 30, 16},   {2, 29, 20},   {3, 28, 24},   {4, 27, 28},
        {5, 26, 36},   {5, 26, 36},   {6, 25, 44},   {6, 25, 44},
        {7, 24, 60},   {7, 24, 60},   {7, 24, 60},   {7, 24, 60},
        {8, 23, 76},   {8, 23, 76},   {8, 23, 76},   {8, 23, 76},
        {9, 22, 92},   {9, 22, 92},   {9, 22, 92},   {9, 22, 92},
        {10, 21, 124}, {10, 21, 124}, {10, 21, 124}, {10, 21, 124},
        {10, 21, 124}, {10, 21, 124}, {10, 21, 124}, {10, 21, 124},
        {11, 20, 156}, {11, 20, 156}, {11, 20, 156}, {11, 20, 156},
        {11, 20, 156}, {11, 20, 156}, {11, 20, 156}, {11, 20, 156},
        {12, 19, 188}, {12, 19, 188}, {12, 19, 188}, {12, 19, 188},
        {12, 19, 188}, {12, 19, 188}, {12, 19, 188}, {12, 19, 188},
        {13, 18, 220}, {13, 18, 220}, {13, 18, 220}, {13, 18, 220},
        {13, 18, 220}, {13, 18, 220}, {13, 18, 220}, {13, 18, 220},
        {14, 17, 252}, {14, 17, 252}, {14, 17, 252}, {14, 17, 252},
        {14, 17, 252}, {14, 17, 252}, {14, 17, 252}, {14, 17, 252},
        {15, 16, 300}, {15, 16, 300}, {15, 16, 300}, {15, 16, 300},
        {15, 16, 300}, {15, 16, 300}, {15, 16, 300}, {15, 16, 300},
        {15, 16, 300}, {15, 16, 300}, {15, 16, 300}, {15, 16, 300},
        {16, 15, 364}, {16, 15, 364}, {16, 15, 364}, {16, 15, 364},
        {16, 15, 364}, {16, 15, 364}, {16, 15, 364}, {16, 15, 364},
        {16, 15, 364}, {16, 15, 364}, {16, 15, 364}, {16, 15, 364},
        {16, 15, 364}, {16, 15, 364}, {16, 15, 364}, {16, 15, 364},
        {17, 14, 428}, {17, 14, 428}, {17, 14, 428}, {17, 14, 428},
        {17, 14, 428}, {17, 14, 428}, {17, 14, 428}, {17, 14, 428},
        {17, 14, 428}, {17, 14, 428}, {17, 14, 428}, {17, 14, 428},
        {17, 14, 428}, {17, 14, 428}, {17, 14, 428}, {17, 14, 428},
        {18, 13, 508}, {18, 13, 508}, {18, 13, 508}, {18, 13, 508},
        {18, 13, 508}, {18, 13, 508}, {18, 13, 508}, {18, 13, 508},
        {18, 13, 508}, {18, 13, 508}, {18, 13, 508}, {18, 13, 508},
        {18, 13, 508}, {18, 13, 508}, {18, 13, 508}, {18, 13, 508},
        {18, 13, 508}, {18, 13, 508}, {18, 13, 508}};
    constexpr static SizeClass<uint16_t> medium[]{
        {18, 13, 508},  {19, 12, 764},  {20, 11, 1020}, {21, 10, 1280},
        {22, 9, 1532},  {23, 8, 2044},  {23, 8, 2044},  {24, 7, 4092},
        {24, 7, 4092},  {24, 7, 4092},  {24, 7, 4092},  {24, 7, 4092},
        {24, 7, 4092},  {24, 7, 4092},  {24, 7, 4092},  {25, 6, 8188},
        {25, 6, 8188},  {25, 6, 8188},  {25, 6, 8188},  {25, 6, 8188},
        {25, 6, 8188},  {25, 6, 8188},  {25, 6, 8188},  {25, 6, 8188},
        {25, 6, 8188},  {25, 6, 8188},  {25, 6, 8188},  {25, 6, 8188},
        {25, 6, 8188},  {25, 6, 8188},  {25, 6, 8188},  {26, 5, 16380},
    ...
    constexpr static SizeClass<uint16_t> large[]{{0, 0, 0}};

  public:
    constexpr static SizeClassArray<uint16_t> SIZES = {0, 0, sizes};
    constexpr static SizeClassArray<uint16_t> SMALL = {4, 0, small};
    constexpr static SizeClassArray<uint16_t> MEDIUM = {256, SMALL.max, medium};
    constexpr static SizeClassArray<uint16_t> LARGE = {0, 0, large};
  };

public:
  using sizes = size_classes<uint16_t, data::SIZES, data::SMALL, data::MEDIUM,
                             data::LARGE>;

  struct FreeList : public ::wap::free_list<uint16_t, sizes> {
  public:
    FreeList() = default;
  };
};
...
#define WAP_SLICE_BIT_FUNCS(type_name)                                         \
  WAP_NODISCARD ::wap::type_name type_name(wap::size_t offset)                 \
      const noexcept {                                                         \
  ...
  WAP_NODISCARD ::wap::type_name type_name##b(wap::size_t offset)              \
      const noexcept {                                                         \
  ...
  WAP_NODISCARD ::wap::type_name type_name##n(wap::size_t offset)              \
      const noexcept {                                                         \
...
#define WAP_SLICE_BIT_BOUNDS_CHECK_FUNCS(type_name)                            \
  WAP_NODISCARD ::wap::type_name type_name(wap::size_t offset)                 \
      const noexcept {                                                         \
    if (WAP_UNLIKELY(voffset_of(offset) + sizeof(::wap::type_name) >           \
                     base_end()))                                              \
      return 0;                                                                \
    return ::wap::bits::le::type_name(voffset_of(offset));                     \
  }                                                                            \
...
#define WAP_SLICE_SAFE_FUNCS                                                   \
  WAP_NODISCARD ::wap::i8 i8(wap::size_t offset) const noexcept {              \
    if (WAP_UNLIKELY(voffset_of(offset) >= base_end()))                        \
      return 0;                                                                \
    return *(::wap::i8 *)(voffset_of(offset));                                 \
  }                                                                            \
  void i8(wap::size_t offset, ::wap::i8 value) const noexcept {                \
...
template <typename R, typename E> using result = std::variant<R, E>;

template <typename SizeT = wap::u16> struct vpointer_t {
  SizeT offset;
};

// strings have an optional inlined performance feature that allows the data to
// be inlined up to the specified InlineMax - sizeof(SizeT).
...
struct Grow {
  ...
  template <uint32_t Min> struct AtLeast {
    constexpr static uint32_t calc(uint32_t current, uint32_t min) noexcept {
...
struct ConstructResult {
  ...
  ConstructResult(u8 *data, u8 *base, u8 *tail, u8 *anEnd)
      : data(data), base(base), tail(tail), end(anEnd) {}
};

template <typename M, typename SizeT = typename M::SizeT> struct Header {
  ...
  constexpr static inline void set_message_size(wap::u8 *buf,
                                                wap::size_t size) noexcept {
    if constexpr (sizeof(SizeT) == 2) {
  ...
  constexpr static inline void set_base_size(wap::u8 *buf,
                                             wap::size_t size) noexcept {
    if constexpr (sizeof(SizeT) == 2) {
  ...
  constexpr static inline void set_size(wap::u8 *buf,
                                        wap::size_t size) noexcept {
    if constexpr (sizeof(SizeT) == 2) {
      wap::bits::le::put_u16(buf, (wap::u16)size);
    } else if constexpr (sizeof(SizeT) == 4) {
  ...
  constexpr static inline wap::u8 *base(wap::u8 *buf) noexcept {
    return buf + SIZE;
  }
  ...
  constexpr static inline wap::u8 *end(wap::u8 *buf) noexcept {
    return buf + size(buf);
  }
...
template <typename M, typename SizeT = typename M::SizeT,
          typename H = Header<M, SizeT>, typename A = wap::allocator>
class Msg {
public:
  using Model = M;
  using Allocator = A;
  ...
  constexpr static const wap::size_t BASE_SIZE_PLUS_SIZE =
      Model::BASE_SIZE + sizeof(SizeT);
  constexpr static const wap::size_t MINIMAL_CONSTRUCT_SIZE =
      Header::SIZE + Model::BASE_SIZE;
  ...
  public:
    ...
    WAP_NODISCARD inline SizeT get_size() const {
      return size & ~(FREE_BIT | PREV_FREE_BIT);
    }

    inline void set_size(SizeT value) {
      size = value | (size & (FREE_BIT | PREV_FREE_BIT));
    }
    ...
    WAP_NODISCARD inline bool is_prev_free() const noexcept {
      return size & PREV_FREE_BIT;
    }

    WAP_NODISCARD inline SizeT get_prev_phys() const noexcept {
      if (is_prev_free())
        return get_prev_phys_unsafe();
      return 0;
    }
  ...
  class Walker {
  public:
    using callback_fn = void(wap::u8 *ptr, uint32_t block_offset,
                             uint32_t ptr_offset, uint32_t size, bool used,
                             void *user);

    constexpr static void print_walker(uint8_t *ptr, uint32_t block_offset,
                                       uint32_t ptr_offset, uint32_t size,
                                       bool used, void *user) {
      (void)user;
      printf("[%s]  offset: %u  size: %u \t(%p)\n", used ? "used" : "free",
             (uint32_t)ptr_offset, (uint32_t)size,
             (void *)Block::from_ptr(ptr));
    }
    ...
    static void walk(callback_fn walker, uint8_t *buffer, void *user) {
      ...
      while (block) {
        ...
        walker(block->to_ptr(), offset, offset + Msg::Block::OVERHEAD, size,
               !is_free, user);
  ...
  template <typename Base, typename Allocator = A> class Root : public Base {
  public:
    inline wap::u8 *data() const noexcept { return data_; }

    inline size_t size() const noexcept { return Header::size(data_); }

    inline size_t base_size() const noexcept {
      return Header::base_size(data_);
    }
  ...
  template <typename R> struct trusted_accessors_builder {
    WAP_SLICE_FUNCS
    ...
    trusted_accessors_builder(R &r, wap::size_t offset)
        : r_(r), offset_(offset) {}
  ...
  struct untrusted_accessors {
    ...
    untrusted_accessors(wap::u8 *base, wap::u8 *base_end)
        : base_(base), base_end_(base_end) {}

    wap::u8 *base_;
    wap::u8 *base_end_;
  };

  template <typename R> struct partial_trusted_slice;
  template <typename R> struct trusted_slice;
  template <typename R> struct untrusted_slice;

  // Fixed and Inner are NOT bounds checked.
  // VPointers are NOT bounds checked.
  template <typename R> struct trusted_block : public trusted_accessors {
    ...
    std::string_view get() {
      return std::string_view((const char *)this->base_ + Block::OVERHEAD,
                              size());
    }
    ...
    Fixed fixed(wap::size_t offset, wap::size_t size) const {
      return Fixed(this->base_ + offset);
    }

    inline std::string_view string(wap::size_t offset) const {
      return root_.string(offset);
    }
    ...
    trusted_block(R &root, ::wap::u8 *base)
        : trusted_accessors(base), root_(root) {}
  ...
  struct partial_trusted_block : public trusted_accessors {
    ...
    partial_trusted_block(R &root, ::wap::u8 *base)
        : trusted_accessors(base), root_(root) {}
  ...
  // VPointers are bounds checked.
  template <typename R> struct untrusted_block : public untrusted_accessors {
    ...
    Fixed fixed(wap::size_t offset, wap::size_t size) {
      return Fixed(this->base_ + offset);
    }
  ...
  // Fixed and Inner are NOT bounds checked.
  // VPointers are NOT bounds checked.
  template <typename R> struct trusted_slice : public trusted_accessors {
    using Fixed = trusted_accessors;
    using Inner = trusted_slice<R>;
    using Block = trusted_block<R>;
    ...
    Inner inner(wap::size_t offset) const {
      return Inner(root_, this->base() + offset);
    }
    ...
    trusted_slice(R &root, ::wap::u8 *base)
        : trusted_accessors(base), root_(root) {}
  ...
  struct partial_trusted_slice : public trusted_accessors {
    ...
    partial_trusted_slice(R &root, ::wap::u8 *base)
        : trusted_accessors(base), root_(root) {}
  ...
  // Fixed and Inner are bounds checked.
  // VPointers are bounds checked.
  template <typename R> struct untrusted_slice : untrusted_accessors {
    using Fixed = untrusted_accessors;
    using Inner = untrusted_slice<R>;
    using Block = partial_trusted_block<R>;
...
#define WAP_VIEW_CTORS(name, slice_name, ...)                                  \
  ...
  name(name &&r) noexcept : root<slice_name, Allocator>(__VA_ARGS__) {         \
    r.clear();                                                                 \
  }                                                                            \
  ...
  template <typename Grow, typename Allocator> struct appender;
  ...
  struct trusted_view : public Root<trusted_accessors, Allocator> {
    ...
    using Inner = trusted_slice<trusted_view<Allocator>>; // Bounds have already
                                                          // been verified
    using Block = trusted_block<trusted_view<Allocator>>;

    inline wap::u8 *voffset(wap::size_t offset) noexcept {
      if (WAP_UNLIKELY(offset < sizeof(SizeT)))
        return nullptr;
    ...
    Inner inner(wap::size_t offset) {
      return Inner(*this, this->base() + offset);
    }

    inline std::string_view string(wap::size_t offset) {
      if (WAP_UNLIKELY(offset < BASE_SIZE_PLUS_SIZE))
        return {};
      const auto data = this->voffset_of(offset);
      if constexpr (sizeof(SizeT) == 2) {
        return std::string_view((const char *)data + sizeof(SizeT),
                                wap::bits::le::u16(data));
      } else if constexpr (sizeof(SizeT) == 4) {
        return std::string_view((const char *)data + sizeof(SizeT),
                                wap::bits::le::u32(data));
      } else if constexpr (sizeof(SizeT) == 8) {
        return std::string_view((const char *)data + sizeof(SizeT),
                                wap::bits::le::u64(data));
      }
      return {};
    }
  ...
  struct partial_trusted_view : public Root<trusted_accessors, Allocator> {
    using Fixed = trusted_accessors; // Bounds have already been verified
    using Inner =
        partial_trusted_slice<partial_trusted_view<Allocator>>; // Bounds have
                                                                // already been
                                                                // verified
    ...
    inline wap::u8 *voffset(wap::size_t offset) noexcept {
      const auto data = this->voffset_of(offset);
      if (WAP_UNLIKELY(data < this->base_ || data > end_ - sizeof(SizeT)))
        return nullptr;
      return data;
    }

    Fixed fixed(wap::size_t offset, wap::size_t size) const {
      return Fixed(this->voffset_of(offset));
    }

    Inner inner(wap::size_t offset) const {
      return Inner(*this, this->voffset_of(offset));
    }
    ...
    std::string_view string(wap::size_t offset) {
      ...
      if constexpr (sizeof(SizeT) == 2) {
        ...
        if (WAP_UNLIKELY(data + size > end_))
          return {};
    ...
    std::optional<Block> optional_block(wap::size_t offset) {
      ...
      if (WAP_UNLIKELY(data < this->base_ || data > end_ - sizeof(Block) ||
                       data + (wap::size_t)((Msg::Block *)data)->get_size() >
                           end_))
        return std::nullopt;
  ...
  struct untrusted_view : public Root<untrusted_accessors, Allocator> {
    ...
    explicit untrusted_view(wap::u8 *data, wap::u8 *base, wap::u8 *base_end,
                            wap::u8 *end)
        : Root<untrusted_accessors, Allocator>(data, base, base_end),
          end_(end) {}

    WAP_VIEW_CTORS(untrusted_view, untrusted_accessors, r.data_, r.base(),
                   r.base_end())
  private:
    wap::u8 *end_;
  };
...
#undef WAP_VIEW_CTORS
  ...
  template <typename Allocator = A>
  using ViewPartial =
      typename Model::template view<partial_trusted_view<Allocator>>;
  template <typename Allocator = A>
  using ViewUntrusted =
      typename Model::template view<untrusted_view<Allocator>>;
  template <typename Allocator = A>
  using ViewVariant =
      std::variant<ViewTrusted<Allocator>, ViewPartial<Allocator>,
                   ViewUntrusted<Allocator>>;

  template <typename Allocator = A>
  inline static wap::result<ViewVariant<Allocator>, wap::ParseError>
  parse(std::string &&s) {
  ...
  template <typename Allocator = A>
  inline static wap::result<ViewVariant<Allocator>, wap::ParseError>
  parse(wap::u8 *data, wap::size_t size) {
    if (WAP_UNLIKELY(!data || size < HEADER_SIZE))
      return wap::ParseError{wap::ParseError::Code::malformed};
    ...
    if (WAP_UNLIKELY(sz > size ||
                     ((wap::size_t)Header::SIZE + base_size > size)))
      return wap::ParseError{wap::ParseError::Code::overflow};

    if (base_size < BASE_SIZE) {
      return ViewVariant<Allocator>{
          std::in_place_type<ViewUntrusted<Allocator>>, data,
          data + Header::SIZE, data + HEADER_SIZE + base_size, data + sz};
    }
    return ViewVariant<Allocator>{std::in_place_type<ViewPartial<Allocator>>,
                                  data, data + Header::SIZE, data + sz};
  }
  ...
  //   {
  //      auto parsed = wap::msg<M, SizeT, H,
  //      wap::allocator::leaking>::parse(data, size);
  //      std::visit(wap::match<Vs...>{std::forward<decltype(visitor)>(visitor)...},
  //      parsed);
  //   }

  template <class... Vs>
  inline static std::optional<wap::ParseError>
  visit(wap::u8 *data, wap::size_t size, Vs... visitor) noexcept {
    auto parsed = parse<wap::allocator::noop>(data, size);

    if (std::holds_alternative<wap::ParseError>(parsed)) {
      return std::get<wap::ParseError>(parsed);
    }
  ...
  template <class... Vs>
  inline static std::optional<wap::ParseError> visit(std::string_view msg,
                                                     Vs... visitor) {
    return visit((wap::u8 *)msg.data(), (wap::size_t)msg.size(), visitor...);
  }

  template <class... Vs>
  inline static void visit_or_throw(wap::u8 *data, wap::size_t size,
                                    Vs... visitor) {
    auto parsed = parse<wap::allocator::noop>(data, size);

    if (std::holds_alternative<wap::ParseError>(parsed)) {
      throw std::get<wap::ParseError>(parsed);
    }
  ...
  inline static ConstructResult construct(size_t extra = BASE_SIZE / 2) {
  ...
  template <typename B> struct block_builder {
    bool deallocate() {
      if (WAP_UNLIKELY(!data_))
        return true;
      b_.deallocate(*vptr_);
      block_ = nullptr;
      data_ = nullptr;
    ...
    wap::u8 *shrink(wap::size_t new_capacity) {
      ...
      if (WAP_UNLIKELY(new_capacity == existing_capacity))
        return data_;
  ...
  struct appender : public Root<trusted_accessors_mutable> {
    using Fixed =
        trusted_accessors_builder<appender<Grow>>; // Bounds have already been
                                                   // verified
    using Inner = trusted_slice<trusted_view<Allocator>>; // Bounds have already
                                                          // been verified
    using Block = trusted_block<trusted_view<Allocator>>;
    ...
    bool grow(wap::size_t by) {
      ...
      // can't shrink
      if (WAP_UNLIKELY(new_capacity < (size_t)capacity()))
        return false;

      // try to reallocate
      const auto new_buffer = Allocator::reallocate(this->data_, new_capacity);
      if (WAP_UNLIKELY(!new_buffer))
        return false;
    ...
    AllocResult append(wap::size_t size) {
      ...
      if (WAP_UNLIKELY(new_tail > end_)) {
        if (WAP_UNLIKELY(!grow(size + Block::OVERHEAD)))
          return {nullptr, 0, 0};
    ...
    AllocResult allocate(wap::size_t size) { return append(size); }
    ...
    void deallocate(wap::size_t offset) {
      ...
      // Out of bounds?
      if (WAP_UNLIKELY(offset < BASE_SIZE_PLUS_SIZE ||
                       offset > current_size - sizeof(Block)))
        return;
      ...
      // Dangling pointer?
      if (WAP_UNLIKELY(blk->is_free()))
        return;

      const auto block_size = blk->get_size();

      // Is it the last allocated block?
      if (WAP_LIKELY(current_size ==
                     offset + Msg::Block::OVERHEAD + block_size)) {
    ...
    void free(wap::u8 *ptr) {
      ...
      if (WAP_UNLIKELY(ptr < base + BASE_SIZE_PLUS_SIZE))
        return;
      deallocate(ptr - base);
    }

    AllocResult reallocate(wap::size_t offset, wap::size_t size) {
      const wap::size_t current_size = this->size();
      ...
      if (WAP_UNLIKELY(!new_allocation.ptr))
        return {nullptr, 0, 0};
      ::memcpy(new_allocation.ptr + Msg::Block::OVERHEAD,
               (void *)(blk + Msg::Block::OVERHEAD),
    ...
    appender(wap::u8 *data, wap::u8 *base, wap::u8 *tail, wap::u8 *end)
        : Root<trusted_accessors_mutable>(data, &this->data_,
                                          (wap::size_t)Header::SIZE),
          tail_(tail), end_(end), trash_(0) {}
  ...
  template <typename Grow = wap::Grow::Doubled> class arena_embedded {};

  template <typename S, typename Grow = wap::Grow::Doubled,
            typename FreeList = wap::Small::FreeList>
  class arena {};
...
//  template <typename Builder, typename View = typename Builder::Finish> View
//  freeze(Builder&& b) {
//    return b.finish();
//  }
...
struct BlockHeader {
  ...
  constexpr static inline BlockHeader *from_ptr(const u8 *ptr) {
    return WAP_CAST(block_header *, ptr - DATA_OFFSET);
  }

  WAP_NODISCARD inline u8 *to_ptr() const noexcept {
    return WAP_CAST(u8 *, this) + DATA_OFFSET;
  }
};

static_assert(sizeof(BlockHeader) == sizeof(uint32_t) * 4);

class walker {
public:
  using callback_fn = void(u8 *ptr, uint32_t block_offset, uint32_t ptr_offset,
  ...
  constexpr static void print_walker(uint8_t *ptr, uint32_t block_offset,
    ...
    printf("[%s]  offset: %u  size: %u \t(%p)\n", used ? "used" : "free",
           (uint32_t)ptr_offset, (uint32_t)size,
           (void *)block_header::from_ptr(ptr));
  }
  ...
  static void walk(callback_fn walker, uint8_t *buffer, void *user) {
    ...
    while (block) {
      ...
      walker(block->to_ptr(), offset, offset + BlockHeader::DATA_OFFSET, size,
             !is_free, user);

      offset += size + BlockHeader::OVERHEAD;

      if (offset >= total_size) {
        break;
      }

      block = (BlockHeader *)WAP_POINTER_OFFSET(buffer, offset);
    }
  }
};

template <typename Allocator = ::wap::allocator> class View {
public:
  using SizeT = BlockHeader::SizeT;
  ...
  WAP_NODISCARD inline SizeT size() const noexcept {
    return SizeT(bits::le::u32(buffer_));
  }
  ...
  public:
    ...
    inline ::wap::u8 *const message_data() const noexcept {
      return view_.buffer_;
    }
    inline ::wap::u32 message_size() const noexcept { return view_.size(); }
    inline ::wap::u32 message_base_size() const noexcept {
      return view_.base_size();
    }
    ...
    Slice inner(::wap::u32 offset, ::wap::u32 bounds) {
      if (WAP_UNLIKELY(offset >= size_))
        return {view_, NULL_SLICE, 0};
      return {view_, (::wap::u8 *)(slice_ + offset),
              WAP_MIN(bounds, size_) - offset};
    }

    ::wap::View<Allocator>::Slice block(wap::u32 offset) {
      // bounds check offset
      if (WAP_UNLIKELY(offset < BlockHeader::OVERHEAD ||
                       offset >= view_.size()))
        return {view_, NULL_SLICE, 0};

      // deref block
      auto block = (BlockHeader *)WAP_POINTER_OFFSET(
          view_.buffer_, offset - BlockHeader::DATA_OFFSET);

      // bounds check block
      const auto block_size = block->get_size();
      if (WAP_UNLIKELY(offset + block_size > view_.size()))
        return {view_, NULL_SLICE, 0};
  ...
  static View<Allocator> make(SizeT base_size, SizeT capacity) {
    ...
    auto &base_block =
        *(BlockHeader *)WAP_POINTER_OFFSET(buffer, (ptrdiff_t)base_offset);
...
class Builder {
public:
  template <typename Buffer> class Slice {
    static const uint32_t NULL_BLOCK_HEADER = 0;
    constexpr static ::wap::u8 *NULL_BLOCK = (::wap::u8 *)&NULL_BLOCK_HEADER;
    constexpr static ::wap::voffset_t *NULL_VPOINTER =
        (::wap::voffset_t *)&NULL_BLOCK_HEADER;
  ...
  public:
    ...
    Slice(Buffer &b, ::wap::u8 *slice, ::wap::u32 size)
        : b_(b), slice_(slice), size_(size) {}

    // Blocks represent a dynamic allocation owned by a virtual pointer address.
    // block_header represents the physical layout which essentially dictates
    // that each block has a 4byte size field. A block may be null as evident by
    // the vpointer containing the 0 value.
    class Block {
    ...
    public:
      ...
      inline u32 capacity() {
        return ::wap::bits::le::u32(data_ - BlockHeader::OVERHEAD) &
               ~(BlockHeader::FREE_BIT | BlockHeader::PREV_FREE_BIT);
      }
      ...
      Block(Buffer &buffer)
          : buffer_(buffer), vptr_((voffset_t *)DATA), data_((wap::u8 *)DATA) {}
    ...
    Slice inner(::wap::u32 offset, ::wap::u32 bounds) {
      if (WAP_UNLIKELY(offset >= size_))
        return {b_, NULL_SLICE, 0};
      return {b_, (::wap::u8 *)(slice_ + offset),
              WAP_MIN(bounds, size_) - offset};
    }

    wap::Builder::Slice<Buffer>::Block block(::wap::u32 voffset,
                                             ::wap::u32 vbounds) {
      if (WAP_UNLIKELY(vbounds > size()))
        return {b_};
      auto vptr = (voffset_t *)WAP_POINTER_OFFSET(slice_, voffset);
      const auto offset = *vptr;

      // bounds check offset
      if (WAP_UNLIKELY(offset < BlockHeader::OVERHEAD || offset >= b_.size())) {
        return {b_};
      }

      auto block_size =
          ::wap::bits::le::u32(b_.data() + offset - BlockHeader::OVERHEAD) &
          ~(BlockHeader::FREE_BIT | BlockHeader::PREV_FREE_BIT);

      if (WAP_UNLIKELY(offset + block_size > b_.size()))
        return {b_};

      return {b_, vptr, b_.data() + offset};
    }

    //      std::optional<wap::builder::block<Buffer>> maybe_block(::wap::u32
    //      voffset,
    //                                                             ::wap::u32
    //                                                             vbounds) {
    //        if (WAP_UNLIKELY(vbounds > size())) return {b_, NULL_VPOINTER,
    //        NULL_BLOCK_HEADER}; const voffset_t* vptr =
    //        (voffset_t*)WAP_POINTER_OFFSET(s_, voffset); const auto offset =
    //        *vptr;
    //
    //        // bounds check offset
    //        if (WAP_UNLIKELY(offset < block_header::OVERHEAD || offset >=
    //        b_.size()))
    ...
    //        NULL_VPOINTER, NULL_BLOCK_HEADER};
    //
    //        return {b_, vptr, blk};
    //      }
  ...
  template <typename Buffer> class Str {
  public:
    ...
    bool operator==(::std::string_view value) {
      ...
      if (!value.data())
        return false;
      if (value.size() != (size_t)block_.capacity())
        return false;
...
template <typename Allocator = ::wap::allocator,
          typename GrowPolicy = ::wap::Grow::Doubled>
class Appender {
public:
  using SizeT = BlockHeader::SizeT;
  ...
  wap::Builder::Slice<Appender<Allocator, GrowPolicy>> base() {
    return wap::Builder::Slice<Appender<Allocator, GrowPolicy>>(
        *this, buffer_ + sizeof(uint32_t) + BlockHeader::OVERHEAD, base_size());
  }

  bool grow(SizeT by) {
    const size_t new_capacity = GrowPolicy::calc(capacity_, by);
    if (WAP_UNLIKELY(new_capacity < (size_t)capacity_))
      return false;
  ...
  AllocResult append(SizeT size) {
    auto const current_size = this->size();
    auto const new_size = current_size + BlockHeader::OVERHEAD + size;

    // Grow?
    if (WAP_UNLIKELY(new_size > capacity_ &&
                     !grow(size + BlockHeader::OVERHEAD))) {
      return {nullptr, 0, 0};
    }
    ...
    const auto data_offset = current_size + BlockHeader::OVERHEAD;

    return {(u8 *)WAP_POINTER_OFFSET(buffer_, data_offset), data_offset, size};
  }

  AllocResult allocate(SizeT size) { return append(size); }

  AllocResult reallocate(voffset_t offset, SizeT size) {
    const SizeT block_offset = offset - BlockHeader::DATA_OFFSET;

    // Out of bounds?
    if (WAP_UNLIKELY(block_offset > capacity_ - BlockHeader::DATA_OFFSET))
      return {nullptr, 0, 0};

    // Get block.
    auto block = (BlockHeader *)WAP_POINTER_OFFSET(buffer_, block_offset);

    // Dangling pointer?
    if (WAP_UNLIKELY(block->is_free()))
      return {nullptr, 0, 0};
    ...
    if (current_size == offset + size) {
      ...
      ::wap::bits::le::put_u32(buffer_ + offset - BlockHeader::OVERHEAD, size);
    ...
    if (WAP_UNLIKELY(!new_allocation.ptr))
      return {nullptr, 0, 0};
    ::memcpy(new_allocation.ptr, block->to_ptr(),
             WAP_MIN(size, current_block_size));

    // Free the previous block and increment the trash.
    block->set_free();
    trash_ += current_block_size + BlockHeader::OVERHEAD;
  ...
  void deallocate(voffset_t offset) {
    ...
    // Out of bounds?
    if (WAP_UNLIKELY(offset > current_size))
      return;
    ...
    if (current_size == offset + block_size) {
      ::wap::bits::le::put_u32(buffer_, offset - BlockHeader::OVERHEAD);
      return;
    }

    trash_ += block_size + BlockHeader::OVERHEAD;
  }
  ...
  void free(const u8 *ptr) {
    ...
    if (WAP_UNLIKELY(offset < 1 || offset > this->tail_))
      return;
  ...
  static Appender make(SizeT base_size, SizeT capacity) {
    ...
    const ptrdiff_t last_offset =
        base_offset + base_block.get_size() + BlockHeader::OVERHEAD;
    const auto last = (BlockHeader *)WAP_POINTER_OFFSET(buffer, last_offset);
    //      auto last_offset_check = WAP_POINTER_DIFF(last, buffer);
    //      (void)last_offset_check;
    SizeT pool_bytes =
        (ptrdiff_t)capacity - last_offset - (ptrdiff_t)BlockHeader::OVERHEAD;
    ...
    ::wap::bits::le::put_u32(buffer, last_offset + BlockHeader::OVERHEAD);
...
// Heap provides linear memory management for a message root. It uses a
// segmented free list with a bitmap to search for a free block of an equal or
// greater size class. The size of the freelist is determined by a size class
// layout.
//
// O(1) memory allocator that can be used in real-time systems. Uses the same
// block layout and linked-list structure as TLSF. However, it differs in the
// segmented list being a single level rather than the larger memory usage of
// the two level list. For messages that are less than 64kb, the freelist
// overhead is only 64bytes and can be optionally embedded in the buffer.
//
// Use this for highly mutable messages that benefit from a full memory manager.
// Always compare against the appender and rebuilding once the trash gets too
// high.
template <typename Allocator = ::wap::allocator,
          typename GrowthPolicy = ::wap::Grow::Doubled,
          typename FreeList = ::wap::Small::FreeList>
class Heap {
public:
  using BlockSizeT = BlockHeader::SizeT;
  ...
  // no copy
  Heap(Heap &b) = delete;
  Heap &operator=(Heap &r) noexcept = delete;

  // ok move
  Heap(Heap &&b) = default;
  Heap &operator=(Heap &&r) noexcept = default;
  ...
  WAP_NODISCARD inline BlockSizeT capacity() const noexcept {
    return capacity_;
  }
...
private:
  ...
  inline bool is_block_inbounds(BlockSizeT offset) {
    return offset > 0 && offset <= tail_;
  }

  inline BlockHeader *deref_unsafe(BlockSizeT offset) {
    return (BlockHeader *)WAP_POINTER_OFFSET(buffer_, (ptrdiff_t)offset);
  }
...
#else
  // dereference a block_header pointer from logical offset.
  inline BlockHeader *deref_safe(BlockSizeT offset) {
    if (WAP_UNLIKELY(!is_block_inbounds(offset)))
      return nullptr;
    return (BlockHeader *)((ptrdiff_t)buffer_ + (ptrdiff_t)offset);
  }
#endif

  // Remove a given block from the free list.
  inline void remove(BlockHeader *block, BlockSizeT offset) {
    ...
    //      if (freelist_->get(cls) == (ListSizeT)WAP_POINTER_DIFF(block,
    //      buffer_)) {
  ...
  inline void insert(BlockHeader *block, BlockSizeT offset) {
    const SizeClass cls = FreeList::pool_size((ListSizeT)block->get_size());
  ...
  explicit Heap(u8 *buffer, BlockSizeT tail, BlockSizeT capacity,
                FreeList *freelist)
      : buffer_(buffer), capacity_(capacity), tail_(tail), freelist_(freelist) {
  }

  inline AllocResult append_internal(const BlockSizeT size,
                                     const SizeClass cls) {
    ...
    // if/when deallocated.
    auto remaining_offset = block_offset + cls.size + BlockHeader::OVERHEAD;
    ...
    const ListSizeT remain_size =
        block->get_size() - (BlockHeader::OVERHEAD + cls.size);
    //      WAP_ASSERT(block->size() == remain_size + block_header::OVERHEAD +
    //      cls.size);
    ...
    // increase size
    set_size(remaining_offset + BlockHeader::OVERHEAD);

    return {block->to_ptr(), block_offset + BlockHeader::DATA_OFFSET,
            block->get_size()};
  }

  inline BlockSizeT get_head(SizeClass cls) {
    return (BlockSizeT)freelist_->get(cls);
  }

  inline void set_size(BlockSizeT size) noexcept {
    ::wap::bits::le::put_u32(buffer_, size);
  }
...
public:
  ...
  inline bool grow(BlockSizeT by) {
    if (WAP_UNLIKELY(!tail_ || !by))
      return false;
    const size_t new_capacity = GrowthPolicy::calc(this->capacity_, by);
    if (WAP_UNLIKELY(new_capacity < (size_t)this->capacity_))
      return false;
    const auto new_buffer = Allocator::reallocate(buffer_, new_capacity);
    if (WAP_UNLIKELY(!new_buffer))
      return false;

    auto tail = (BlockHeader *)WAP_POINTER_OFFSET(buffer_, tail_);
    ...
    if (WAP_UNLIKELY(!tail->is_free())) {
      ...
      tail = (BlockHeader *)WAP_POINTER_OFFSET(buffer_, tail_);
      tail->set_size(by);
      tail->set_prev_free();
      tail->set_prev_used();
      tail->prev_free = 0;
  ...
  BlockSizeT debug_get_trimmed_size() {
    if (!is_block_inbounds(tail_))
      return this->capacity_;
    const auto &tail = *(BlockHeader *)WAP_POINTER_OFFSET(buffer_, tail_);
    if (WAP_LIKELY(tail.is_free())) {
      // If the last block is free then just return the offset to the last
      // block.
      return tail_ + BlockHeader::OVERHEAD;
    }
    // Return the offset to encompass the last size, but NOT the available size.
    return tail_ + BlockHeader::DATA_OFFSET + tail.get_size();
  }
  ...
  AllocResult append(BlockSizeT size) {
    const SizeClass cls = FreeList::alloc_size((ListSizeT)size);

    // Is the size too big?
    if (WAP_UNLIKELY(!cls.size && !grow(size + BlockHeader::OVERHEAD))) {
      return {nullptr, 0, 0};
    }

    auto block = deref_unsafe(tail_);
    if (WAP_UNLIKELY((!block->is_free() ||
                      block->get_size() - size - BlockHeader::DATA_OFFSET <
                          FreeList::get_min_split())) &&
        !grow(size + BlockHeader::OVERHEAD)) {
      return {nullptr, 0, 0};
    }
  ...
  AllocResult allocate(const BlockSizeT size) {
    const SizeClass cls = FreeList::alloc_size((ListSizeT)size);

    // Is the size too big?
    if (WAP_UNLIKELY(!cls.size))
      return {nullptr, 0, 0};
    ...
    if (WAP_UNLIKELY(!free_class.size)) {
      ...
      if (WAP_UNLIKELY(!grow(this->capacity_)))
        return {nullptr, 0, 0};
      return append_internal(size, cls);
    }

    //      WAP_ASSERT(free_class.size >= cls.size
    //                 && "search_class returned a smaller class than
    //                 requested");

    // Get the block.
    auto block_offset = (BlockSizeT)freelist_->get(free_class);
    BlockHeader *block = deref_safe(block_offset);
    ...
    BlockHeader *prev =
        block->prev_free ? deref_safe(block->prev_free) : nullptr;
    BlockHeader *next =
        block->next_free ? deref_safe(block->next_free) : nullptr;
    if (next) {
      if (!prev) {
        next->prev_free = 0;
      } else {
    ...
    if (WAP_LIKELY(free_class.size > cls.size &&
      ...
      const ListSizeT remain_size =
          block->get_size() - (BlockHeader::OVERHEAD + cls.size);
      WAP_ASSERT(block->get_size() ==
                 remain_size + BlockHeader::OVERHEAD + cls.size);
      ...
      next =
          deref_safe(block_offset + block->get_size() + BlockHeader::OVERHEAD);
      if (next) {
        next->set_prev_used();
      }
    }
  ...
  void deallocate(const BlockSizeT offset) {
    if (!WAP_UNLIKELY(offset))
      return;
    const auto block_offset = offset - BlockHeader::DATA_OFFSET;
    BlockHeader *block = deref_safe(block_offset);

    //      WAP_ASSERT(!block->is_free() && "block already marked as free");

    // Get next adjacent block.
    auto next_offset = offset + block->get_size() - BlockHeader::OVERHEAD;
    if (is_block_inbounds(next_offset)) {
      ...
      if (next->is_free()) {
        ...
        // Increase size.
        block->size += next->get_size() + BlockHeader::OVERHEAD;

        if (next_offset == tail_) {
          tail_ = block_offset;
          set_size(block_offset + BlockHeader::OVERHEAD);
        } else {
        ...
        next->prev_phys = offset - BlockHeader::DATA_OFFSET;
        next->set_prev_free();
      }
    }
    ...
    if (block->is_prev_free()) {
      BlockHeader *prev = deref_safe(block->prev_phys);
      WAP_ASSERT(prev && prev->is_free() &&
                 "prev block is not free though marked as such");
      remove(prev, block->prev_phys);

      if (block_offset == tail_) {
        tail_ = block->prev_phys;
        set_size(block->prev_phys + BlockHeader::OVERHEAD);
      } else {
        // Link next physical block to new previous block.
        next_offset =
            block->prev_phys + BlockHeader::DATA_OFFSET + prev->get_size();
        if (WAP_LIKELY(is_block_inbounds(next_offset))) {
          BlockHeader *next = deref_unsafe(next_offset);
          next->prev_phys = block->prev_phys;
        }
      }
      prev->size += block->get_size() + BlockHeader::OVERHEAD;
      block = prev;
    } else {
      block->set_free();
    }
  ...
  void free(u8 *ptr) {
    const auto offset = WAP_POINTER_DIFF(ptr, buffer_);
    if (WAP_UNLIKELY(offset < 1 ||
                     offset > this->tail_ + BlockHeader::DATA_OFFSET))
      return;
    deallocate((voffset_t)offset);
  }

  // Reallocate / resize an existing allocation.
  AllocResult reallocate(const BlockSizeT offset, const BlockSizeT size) {
    auto block_offset = offset - BlockHeader::DATA_OFFSET;
    ...
    if (WAP_LIKELY(is_block_inbounds(next_offset))) {
      ...
      if (next->is_free()) {
        const auto next_size = next->get_size() + BlockHeader::OVERHEAD;

        // Split?
        if (WAP_LIKELY(next_size >= extend_size + FreeList::get_min_size() +
                                        BlockHeader::OVERHEAD)) {
          // Remove from freelist.
          remove(next, next_offset);

          // Reserve by the size class size to promote better free list cache
          // hits if/when deallocated.
          auto remaining_offset = offset + extend_size + BlockHeader::OVERHEAD;
          auto remaining = deref_unsafe(remaining_offset);
          ...
          // Calculate the remaining size.
          const ListSizeT remain_size =
              block->get_size() - (BlockHeader::OVERHEAD + extend_size);
          WAP_ASSERT(block->get_size() ==
                     remain_size + BlockHeader::OVERHEAD + extend_size);
  ...
  static Heap make(BlockSizeT base_size, BlockSizeT capacity) {
    auto buffer = (u8 *)Allocator::allocate((size_t)capacity);
    memset(buffer, 0, capacity);

    ptrdiff_t base_offset = 0; //-4;  // sizeof(uint32_t);
    //      auto& base_block = *(block_header*)WAP_POINTER_OFFSET(buffer,
    ...
    const ptrdiff_t freelist_offset =
        base_offset + base_block.get_size() + BlockHeader::OVERHEAD;
    auto &freelist_block =
        *(BlockHeader *)WAP_POINTER_OFFSET(buffer, freelist_offset);
    freelist_block.set_size(sizeof(FreeList));
    freelist_block.set_used();
    auto freelist = (FreeList *)freelist_block.to_ptr();

    const ptrdiff_t last_offset =
        freelist_offset + BlockHeader::OVERHEAD + freelist_block.get_size();
    const auto last = (BlockHeader *)WAP_POINTER_OFFSET(buffer, last_offset);
    auto last_offset_check = WAP_POINTER_DIFF(last, buffer);
    (void)last_offset_check;
    ListSizeT pool_bytes =
        (ptrdiff_t)capacity - last_offset - (ptrdiff_t)BlockHeader::OVERHEAD;
    ...
    return Heap(buffer, last_offset, capacity, freelist);
  }
...
template <typename Allocator = ::wap::allocator,
          typename FreeListT = ::wap::Small::FreeList>
struct HeapExternFreelist : public Heap<Allocator, FreeListT> {
  using SizeT = BlockHeader::SizeT;

public:
  HeapExternFreelist() : HeapExternFreelist<Allocator, FreeListT>(&freelist_) {}
